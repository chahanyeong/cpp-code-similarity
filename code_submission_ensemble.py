...
def data_preprocess(args):
    ...
    # 위치 변경
    processed_test.to_csv("./data/new_dataset_0604/processed_test.csv", index=False) #이것도 항상할 필요 없음
    ...
    for problem in tqdm(problems):
        ...
        positive_pairs = positive_pairs[:900]
        random.shuffle(other_codes)
        other_codes = other_codes[:900]
    ...
    for problem in tqdm(problems):
        ...
        positive_pairs = list(combinations(solution_codes,2))
        random.shuffle(positive_pairs)
        positive_pairs = positive_pairs[:40]
        random.shuffle(other_codes)
        other_codes = other_codes[:40]
...
def train_model(args):
    ...
    dacon_train_data = pd.read_csv("./data/" + "new_dataset_0607/dacon_train_random.csv")
    dacon_valid_data = pd.read_csv("./data/" + "new_dataset_0607/dacon_valid_random.csv")

    train_data = dacon_train_data #수정
    valid_data = dacon_valid_data #수정
...
def model_ensemble():
    ...
    submission_1 = pd.read_csv('submission_graphcodebert_random.csv')
    submission_2 = pd.read_csv('submission_codebert-cpp_random.csv')
    ...
    submission.to_csv('submission_ensemble_random_graph_cobertcpp각180만_0.5=1.csv', index=False)
